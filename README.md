...
| 機能 | 内容 |
|------|------|
| トレース取得 | Langfuse API を並列ページネーションで全件取得 |
| Observation 取得 | LangGraph 子スパンを一括取得し、root trace が null の場合のフォールバック元として使用 |
...
@tool デコレーター
  └─ export_langfuse_sessions() -> bytes   ← ツールのエントリーポイント
       ...
       ├─ _fetch_all_traces()              ← 並列ページネーションで全件取得
       ├─ _fetch_all_observations()        ← LangGraph 子スパンを一括取得 → {traceId: [obs]}
       ├─ _aggregate_sessions()            ← セッション単位に集計（observation フォールバックあり）
...
#### Langfuse トレースフォーマット変更への対応（2026/05 〜）

サービス稼働開始後、wxO サーバーの更新により root trace の `input`/`output` が `null` になり、ユーザー発言・エージェント応答・エージェント名が root trace から取得できなくなった。これらのデータは `LangGraph` 子スパン（observation）にのみ記録される。

```
【変更前】
root trace
  ├─ input: { current_agent_id, agent_display_name, messages: [{role: user, ...}] }
...
root trace
  ├─ input: null
  └─ output: null
LangGraph span (observation)
  ├─ input: { current_agent_id, agent_display_name, messages: [{role: user, ...}] }
  └─ output: { messages: [{role: assistant, ...}] }
```

対応として `_get_user_message` / `_get_assistant_message` / `_get_agent_info` に observation フォールバックを追加。root trace に値がある場合はそのまま使用し、`null` の場合のみ子スパンから取得するため、旧フォーマットのデータも引き続き正しく処理される。
...
| 処理 | `export_traces.py` | `langfuse_sessions_tool.py` |
|------|-------------------|----------------------------|
| トレース取得 | `fetch_all_traces()` | `_fetch_all_traces()` |
| Observation 取得 | `fetch_all_observations()` | `_fetch_all_observations()` |
| セッション集計 | `export_sessions_csv()` | `_aggregate_sessions()` |
| フォールバック | root trace → LangGraph 子スパン | root trace → LangGraph 子スパン |
| 出力形式 | CSV ファイルをローカルに書き出し | bytes（Excel）を wxO に返す |
| 認証 | `.env` から `os.getenv()` | wxO Connection から取得 |

両ファイルは同等のトレース取得・集計ロジックを持つ。`export_traces.py` はローカル実行・動作確認用として引き続き利用可能。
